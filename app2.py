...
# -------------------------- Sidebar Filters --------------------------
st.sidebar.image("assets/xia.png", use_container_width=True)
st.sidebar.image("assets/WUT-Logo.png", use_container_width=True)
st.sidebar.write("**Name:** ziyi Xia")
st.sidebar.write("**Github:** [https://github.com/xiaziyi36/Data-visualization-analysis-warehouse](https://github.com/xiaziyi36/Data-visualization-analysis-warehouse)")
...
# 关键：自动获取桌面路径，适配Windows/macOS系统
import os
# 获取当前用户的桌面路径（无需手动修改用户名）
desktop_path = os.path.join(os.path.expanduser("~"), "Desktop")
# 拼接完整的CSV保存路径（文件名为固定名称，可自行修改）
csv_save_path = os.path.join(desktop_path, "cleaned_filtered_tourism_customer_behavior_data.csv")

# 生成CSV数据并保存到桌面
csv_data = filtered_df.to_csv(index=False, encoding='utf-8-sig')  # utf-8-sig兼容中文显示
# 1. 自动保存到桌面（无需用户点击，运行脚本即生成）
with open(csv_save_path, 'w', newline='', encoding='utf-8-sig') as f:
    f.write(csv_data)
st.success(f"✅ 数据已自动保存到桌面：{csv_save_path}")

# 2. 保留原有的手动下载按钮（双重保障，用户也可手动点击下载）
st.download_button(
    label="手动下载数据（备份）",
    data=csv_data,
